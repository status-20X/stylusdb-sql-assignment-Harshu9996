...
async function executeSELECTQuery(query) {
    ...
    if(joinTable && joinCondition){
        ...
        switch (joinType.toUpperCase()) {
            case 'INNER':
                data = performeInnerJoin(
                    data, joinData, joinCondition, fields, table
                );
                break;
            ...
            default: throw new Error(`${joinType} not supported`);
                break;
        }
...
function performeInnerJoin( data, joinData, joinCondition, fields, table){
    return data.flatMap(mainRow=> {
        return joinData.filter(joinRow=>{
...
function performLeftJoin( data, joinData, joinCondition, fields, table){
    ...
    //             const mainValue = mainRow[joinCondition.left.split('.')[1]];
    //             const joinValue = joinRow[joinCondition.right.split('.')[1]];
    //             return mainValue === joinValue || mainValue; // Include unmatched rows for left join
    //         })
    //         .map(joinRow => {
    //             return fields.reduce((acc, field) => {
    //                 const [tableName, fieldName] = field.split('.');
...
function performRightJoin( data, joinData, joinCondition, fields, table){
    ...
    //             const mainValue = mainRow[joinCondition.left.split('.')[1]];
    //             const joinValue = joinRow[joinCondition.right.split('.')[1]];
    //             return mainValue === joinValue || joinValue; // Include unmatched rows for left join
    //         })
    //         .map(joinRow => {
    //             return fields.reduce((acc, field) => {
    //                 const [tableName, fieldName] = field.split('.');
